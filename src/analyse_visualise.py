# analyse_visualise.py  — minimal sweep (no saving)
...
def _num_blocks(model: PreTrainedModel) -> int:
    # Try common layouts to count transformer blocks
...
def sweep_patch_layers(
    ...
    layers: Optional[Iterable[int]] = None, # e.g., range(5) for first 5 layers
    progress: bool = True,
) -> List[float]:
    """
    ...
    Returns a list of per-layer deltas.
    """
    t_start = time.time()
    ...
    # Baseline probability for "Yes" at the final token (target item)
    with torch.inference_mode():
        logits_base = model(**inputs_on_device).logits
    p_yes_base = F.softmax(logits_base[target_idx, -1], dim=-1)[yes_id].item()
    ...
    for j, layer_idx in enumerate(layer_list):
        ...
        with torch.inference_mode():
            patched_logits = run_with_patched_activation(
                ...
                inputs_on_device=inputs_on_device,  # reuse tensors already on device
            )
