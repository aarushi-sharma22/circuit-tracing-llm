...
def run_with_patched_activation(
    ...
    """
    - 'mlp_out': patch final MLP projection inside block[layer_idx] via forward hook.
    - 'resid_out': patch residual stream entering block[layer_idx] by pre-hooking
      its first LayerNorm/RMSNorm. Prefers cache 'resid_pre.{layer_idx}'; falls back
      to 'resid_out.{layer_idx}' and patches next block input.
    Hooks are always removed (try/finally).
    """
    ...
    if kind == "resid_out":
        if f"resid_pre.{layer_idx}" in cache_source:
            cache_key = f"resid_pre.{layer_idx}"; where = "this_block_pre_norm"
        elif f"resid_out.{layer_idx}" in cache_source:
            cache_key = f"resid_out.{layer_idx}"; where = "next_block_pre"
        else:
            raise KeyError(f"Cache missing 'resid_pre.{layer_idx}' and 'resid_out.{layer_idx}'. Rebuild cache.")
    else:
        cache_key = f"{kind}.{layer_idx}"; where = None
